...
public class CDictionaryStringKey<V> extends CGenericArray<CKeyValueEntry<String,V>>
{
    ...
    public V getValue(String p_sKey) 
    {
        ...
        CKeyValueEntry<String,V> oFoundEntry = this.FindEntry(p_sKey);
        if (oFoundEntry != null)
    ...
    public void setValue(String p_sKey, V p_oValue) 
    {
        ...
        CKeyValueEntry<String,V> oEntry = this.EnsureEntry(p_sKey);
    ...
    //--------------------------------------------------------------------------
    public CDictionaryStringKey() 
    {
        super(1024);
        this.isOrdered = true;
    }
    //--------------------------------------------------------------------------
    public CDictionaryStringKey(boolean p_bIsOrdered) 
    {
        super(1024);
        this.isOrdered = p_bIsOrdered;
    }    
    //--------------------------------------------------------------------------
    public CDictionaryStringKey(int p_nPageSize, boolean p_bIsOrdered) 
    {
        super(p_nPageSize);
        this.isOrdered = p_bIsOrdered;
    }
    //--------------------------------------------------------------------------
    public CKeyValueEntry<String,V> FindEntry(String p_sKey) 
    {
        if (this.itemCount == 0)
            return null;
    ...
    public CKeyValueEntry<String,V> exhaustiveSearch(String p_sKey) 
    {
        CKeyValueEntry<String,V> oFoundEntry = null;
        CKeyValueEntry<String,V> oEntry;
        for(int i = 0; i < this.itemCount; i++)
        {
    ...
    //--------------------------------------------------------------------------
    public CKeyValueEntry<String,V> recurseBinarySearch(String p_sKey, 
        int p_nStartIndex, int p_nEndIndex, int p_nAlgorithmStep) 
    {        
        CKeyValueEntry<String,V> oResult = null;

        int nMiddleIndex = p_nStartIndex +(p_nEndIndex - p_nStartIndex) / 2;  
        CKeyValueEntry<String,V> oEntry = this.getItem(nMiddleIndex);
    ...
    {
        CKeyValueEntry<String,V> oEntry = this.FindEntry(p_sKey);
        return oEntry.Index;
    }    
    //--------------------------------------------------------------------------
    public void AddEntry(CKeyValueEntry<String,V> p_oEntry)
    {
                   
        if (this.isOrdered)
        {
            CKeyValueEntry<String,V> oCurrentEntry = null;
            int nInsertionPos = 0;
    ...
    public CKeyValueEntry<String,V> EnsureEntry(String p_sKey) 
    {
        ...
        if (oEntry == null)
        {
            oEntry = new CKeyValueEntry<String,V>(p_sKey);
    ...
    public String toString()
    {
        ...
        for(int i=0; i < this.itemCount; i++)
        {
            ...
            CKeyValueEntry<String,V> oEntry = this.getItem(i);
